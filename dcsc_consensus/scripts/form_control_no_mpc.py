...
class Controller:
	...
	def control(self):
		...
		while not rospy.is_shutdown():
			...
			while not self.start:
				...
				dy = rospy.get_param('offset_y',0)
				pass
			#Calculate desired v and w
			v, w = self.calc_vel()
			#v, w = self.avoid(v,w)  
			v, w = self.bump(v,w)

			#Define error to VL
			e = (self.xr[0]+self.dl[0]-self.x[0])**2+(self.xr[1]+self.dl[1]-self.x[1])**2
			if e > 0.05:			
	...
	def calc_vel(self):
		w = self.kv[1]*(arctan2(self.xr[1]+self.dl[1]-self.x[1], self.xr[0]+self.dl[0]-self.x[0]) - self.x[2])
		if abs(w) > 1.5:
			#v = 0
			v = self.kv[0]*(math.sqrt((self.xr[0]+self.dl[0]-self.x[0])**2+(self.xr[1]+self.dl[1]-self.x[1])**2))
		else:		
			v = self.kv[0]*(math.sqrt((self.xr[0]+self.dl[0]-self.x[0])**2+(self.xr[1]+self.dl[1]-self.x[1])**2))
		return v, w
